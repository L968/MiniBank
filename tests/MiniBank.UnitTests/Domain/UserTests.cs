...
{
    [Fact]
    public void ValidateCanSendMoney_ShouldThrowException_WhenUserIsNotCommon()
    {
        // Arrange
        var user = new User("Merchant User", "12345678901", "merchant@example.com", "hashedPassword", UserType.Merchant);

        // Act & Assert
        AppException exception = Assert.Throws<AppException>(() => user.ValidateCanSendMoney(100));
        Assert.Equal("Only Common users are allowed to send money.", exception.Message);
    }

    [Fact]
    public void ValidateCanSendMoney_ShouldThrowException_WhenBalanceIsInsufficient()
    {
        // Arrange
    ...
    [Fact]
    public void ValidateCanSendMoney_ShouldAllowToSendMoney_WhenUserIsCommonAndHasEnoughBalance()
    {
        // Arrange
        var user = new User("Common User", "98765432100", "user@example.com", "hashedPassword", UserType.Common);
        user.Credit(100);
